...
def load_corpus_each_sentence(file_name):
    logging.info("begin to load")
    ret = []
    fd = open(file_name, 'r') 
    count = 0
    for line in fd:
        words_list = []
        data_dict = ujson.loads(line.strip())
        sentences = data_dict.get('sentences', [])
        for sent in sentences:
            words_list.append(sent.get('words', []))
        ret.append(words_list)
        if count % 1000 == 0:
            logging.info("load group num:%s", count)
        count += 1
        ...
        for sent in doc:
            ...
            for term in sent:
                if term in self.term_id_map:
                    id = self.term_id_map[term]
                else:
                    id = self.term_to_id(term)
                    if id:
                        self.term_id_map[term] = id
                if id != None:
                    list.append(id)
                    if id not in words:
                        words.add(id)
